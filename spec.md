...
**Status:** Draft — 2026-09-04: the hash encoding is [DASL](https://dasl.ing/) (DRISL + MASL); everything else is the 2026-08-24 draft. The reference implementation (`@estoc/folder-object` 0.7, estoc branch `dasl`) implements this draft and reads nothing else. Every root changes (`bafyrei…` where it was `bafybei…`), `ipfs add` no longer reproduces it, every block remains an IPLD block. Not frozen; subject to incompatible change. Appendix A lists what moved.
...
**What the switch touched.** Downstream, the code that reads blocks: the object-share protocol's skeleton (one block now — the manifest — and the `index.json` leaf; roughly ten thousand entries fit the inline budget), the vault's block store (no 1 MiB bound on a raw block; a drisl block is sound when it is one canonical DRISL document whose links are DASL CIDs — the *manifest* shape is this format's judgment, not the store's, so that other DRISL blocks are not damage), the CID parser every layer shares (it accepts 0x71 and nothing dag-pb), and the CLI's roots. One encoder and one strict decoder — `@estoc/dasl` — serve every reader, so that the same tree cannot root differently in two places.

**No migration.** Every root produced under the UnixFS draft — `bafybei…`, codec 0x70 — is a pre-DASL identity that nothing in this draft computes or reads: a card over one is malformed here, a block store of this draft holds no dag-pb block, and no reader of this draft reads both encodings. What existed under the earlier draft is re-signed under this one. This document only says that the two identities are unrelated: the same mapping has one root under each.
