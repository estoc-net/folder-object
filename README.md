...
Status: **Draft 1.0**, DASL encoding branch (not frozen; the encoding is prototyped as `@estoc/folder-object/dasl` on the estoc `dasl` branch). Version 1 covers internal files only; cross-object references (the `objects` dependency table) are introduced in a later version, with their names already reserved.
